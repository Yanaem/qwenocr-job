...
# Version fonctionnelle de la phase 1. Elle est persistée dans les statistiques
# et dans le cache de progression afin de ne jamais mélanger des pages produites
# avec des prompts ou des réglages incompatibles.
PHASE1_PIPELINE_VERSION = os.getenv(
    "PHASE1_PIPELINE_VERSION",
    "phase1-document-neutral-v1",
)
OCR_PROMPT_VERSION = os.getenv(
    "OCR_PROMPT_VERSION",
    "ocr-layout-neutral-v1",
)
MARKDOWN_PROMPT_VERSION = os.getenv(
...
# Un ancien fichier .progress.json ne contient pas de fingerprint. Il est
# ignoré par défaut pour éviter de fusionner l'ancien format orienté facture
# et le nouveau format documentaire neutre.
ALLOW_LEGACY_PROGRESS = _env_bool("ALLOW_LEGACY_PROGRESS", False)
...
OCR_PROMPT = """Tu es un moteur OCR layout-aware généraliste pour documents administratifs, comptables, contractuels, bancaires, sociaux et commerciaux.
...
- Tu ne classes pas le document comme facture ou non-facture.
- Un document peut être notamment une facture, un avoir, un contrat, un bail,
  un devis, une proforma, un bon de commande, un bon de livraison, un ticket,
  un relevé, un RIB, un courrier, des CGV, une attestation, un bulletin de paie
  ou un autre document administratif.
- Les noms techniques historiques invoice_title, invoice_details, line_items,
  line_items_note, line_items_footer et totals_summary décrivent uniquement la
  structure visuelle. Ils ne signifient jamais que le document est une facture.
- Le titre réellement visible prime toujours sur le nom technique du role_hint.
...
FIDÉLITÉ DES VALEURS, MONTANTS ET IDENTIFIANTS
...
- Nom commercial, raison sociale, organisme, employeur, bailleur, prestataire ou logo textuel de la partie émettrice : role_hint=supplier_identity.
...
- Nom du client, acheteur, destinataire, salarié, preneur, bénéficiaire ou partie facturée : role_hint=customer_identity.
- Adresse de la partie destinataire, du client, du salarié, du preneur ou adresse de facturation : role_hint=customer_address ou billing_address.
- Contact, email, téléphone ou personne de contact de la partie destinataire : role_hint=customer_contact.
- SIRET, TVA intra, identifiant fiscal ou information légale de la partie destinataire : role_hint=customer_legal.
...
- Titre principal du document : FACTURE, AVOIR, CONTRAT, BAIL, DEVIS, PROFORMA, BON DE COMMANDE, BON DE LIVRAISON, TICKET, RELEVÉ, RIB, COURRIER, CGV, ATTESTATION, BULLETIN DE PAIE, REÇU, etc. : role_hint=invoice_title.
- Numéro, date, période, référence, commande, vendeur, page imprimée, devise, objet, code client, statut ou autre métadonnée du document : role_hint=invoice_details.
- Statut de paiement dans la zone des totaux : role_hint=totals_summary.
- Tableau principal réellement visible : articles, prestations, opérations, échéances, mouvements, lignes de paie ou autre grille structurée : role_hint=line_items.
- Note située au début ou au-dessus du tableau principal, liée à son contenu mais ne décrivant pas une ligne de données : role_hint=line_items_note.
- Pied du tableau principal, report, page, contact, signature ou total de report situé en bas du tableau : role_hint=line_items_footer.
...
TABLEAUX — CONTENU PRINCIPAL ET LIGNES COMMERCIALES
...
RÈGLES DOCUMENTAIRES ET FINANCIÈRES
...
SYSTEM_PROMPT_MD = """Vous êtes un assistant spécialisé dans la conversion fidèle d'un OCR layout-aware de documents administratifs, comptables, contractuels, bancaires, sociaux et commerciaux en Markdown.
...
- Ne détermine pas le type métier du document et ne suppose jamais qu'il s'agit
  d'une facture.
- Les role_hint invoice_title, invoice_details, line_items,
  line_items_note, line_items_footer et totals_summary sont des noms techniques
  historiques. Ils indiquent une fonction de mise en page, pas une catégorie.
- Conserve littéralement les titres visibles comme CONTRAT, BAIL, DEVIS,
  RELEVÉ, RIB, BULLETIN DE PAIE, FACTURE ou AVOIR.
- Les titres de sections Markdown sont des conteneurs neutres ajoutés par la
  phase 1. Ils ne doivent jamais remplacer ou reformuler le titre du document.
...
6. En cas de doute de classement : Mentions, Clauses, Signatures et Notes.
...
## Contenu Structuré Principal
## Montants, Taxes et Totaux
## Informations de Paiement
## Mentions, Clauses, Signatures et Notes
...
Vers "Informations Destinataire" :
- customer_identity
- customer_address
- customer_contact
- customer_legal
...
Vers "Titre et Métadonnées du Document" :
- invoice_title
- invoice_details
- unknown si le contenu est clairement un titre, une page imprimée, un numéro, une date, une référence, une commande, un vendeur, une devise, un code client, un statut de paiement ou un objet du document

Vers "Contenu Structuré Principal" :
- line_items_note
- line_items
- line_items_footer

Vers "Montants, Taxes et Totaux" :
- tax_summary
- totals_summary
...
Vers "Mentions, Clauses, Signatures et Notes" :
- legal_terms
- marketing_badge
...
- Ne place dans "Informations Destinataire" que le destinataire, facturé à, acheteur, contact client, information légale client ou son adresse de facturation.
- Ne place pas shipping_address, shipping_details, shipping_contact ou delivery_confirmation dans "Informations Destinataire" si la section "Informations de Livraison" existe.
...
- Ne place jamais un slogan, badge SAV, label qualité, tampon, QR code, texte marketing ou logo secondaire dans "Informations Destinataire".
...
- marketing_badge, stamp_signature, qr_barcode_text vont dans "Mentions, Clauses, Signatures et Notes".
...
- logo_text va dans "Mentions, Clauses, Signatures et Notes" s'il ne permet pas d'identifier l'émetteur.
...
- Un texte isolé d'en-tête sans libellé clair va dans "Titre et Métadonnées du Document" ou dans "Mentions, Clauses, Signatures et Notes", jamais dans le client par défaut.
...
- customer -> Informations Destinataire
- payment -> Informations de Paiement
- logo_marketing -> Mentions, Clauses, Signatures et Notes, sauf si le bloc contient clairement le nom/logo de l'émetteur
- unknown -> classer par contenu explicite ; sinon Mentions, Clauses, Signatures et Notes
...
TABLEAUX — RÉPARATION DES LIGNES DU CONTENU PRINCIPAL
...
- Le tableau des articles/prestations doit rester dans "Contenu Structuré Principal".
...
- Les blocs marketing, SAV, logo secondaire, QR code, tampon et slogans ne sont ni dans Informations Destinataire ni dans Informations Émetteur.
- Les informations de livraison sont dans "Informations de Livraison", pas dans "Informations Destinataire", quand elles sont distinguables.
- customer_contact et customer_legal sont dans Informations Destinataire.
...
def get_pipeline_fingerprint() -> str:
    """Empreinte stable des réglages qui influencent le contenu produit."""
    payload = {
        "pipeline_version": PHASE1_PIPELINE_VERSION,
        "ocr_prompt_version": OCR_PROMPT_VERSION,
        "markdown_prompt_version": MARKDOWN_PROMPT_VERSION,
        "model_ocr": MODEL_OCR,
        "model_md": MODEL_MD,
        ...
        "markdown_prompt": SYSTEM_PROMPT_MD,
    }
...
def load_progress(pdf_path: str) -> Dict[str, Dict]:
    ...
    try:
        ...
        if isinstance(data, dict) and "pages" in data and isinstance(data["pages"], dict):
            stored_fingerprint = str(data.get("pipeline_fingerprint") or "").strip()
            if stored_fingerprint == PIPELINE_FINGERPRINT:
                return data["pages"]

            if not stored_fingerprint and ALLOW_LEGACY_PROGRESS:
                _log("⚠️ Reprise d'un cache legacy sans fingerprint (ALLOW_LEGACY_PROGRESS=true).")
                return data["pages"]

            _log(
                "⚠️ Cache de progression ignoré : pipeline ou prompts incompatibles "
                f"(stocké={stored_fingerprint or 'legacy'}, courant={PIPELINE_FINGERPRINT[:12]}...)."
            )
            return {}

        if isinstance(data, dict):
            if ALLOW_LEGACY_PROGRESS:
                _log("⚠️ Reprise d'un ancien cache plat (ALLOW_LEGACY_PROGRESS=true).")
                return data
            _log("⚠️ Ancien cache plat ignoré : aucun fingerprint de pipeline.")
            return {}
        return {}
    except Exception:
        return {}
...
LEGACY_TO_NEUTRAL_HEADINGS = {
    "## Informations Émetteur (Fournisseur)": "## Informations Émetteur",
    "## Informations Client": "## Informations Destinataire",
    "## Détails de la Facture": "## Titre et Métadonnées du Document",
    "## Tableau des Lignes de Facturation": "## Contenu Structuré Principal",
    "## Montants Récapitulatifs": "## Montants, Taxes et Totaux",
    "## Mentions Légales et Notes Complémentaires": "## Mentions, Clauses, Signatures et Notes",
}
LEGACY_TO_NEUTRAL_HEADINGS_CASEFOLD = {
    key.casefold(): value for key, value in LEGACY_TO_NEUTRAL_HEADINGS.items()
}


def _neutralize_generated_headings(md: str) -> str:
    """
    Remplace uniquement les titres techniques historiques ajoutés par la phase 1.

    Les titres réellement présents dans le document (FACTURE, CONTRAT, BAIL,
    DEVIS, etc.) ne sont jamais modifiés, car seuls des titres Markdown complets
    et exactement égaux aux anciennes sections sont remplacés.
    """
    lines: List[str] = []
    for line in (md or "").splitlines():
        stripped = line.strip()
        replacement = LEGACY_TO_NEUTRAL_HEADINGS_CASEFOLD.get(stripped.casefold())
...
def markdown_from_ocr(api_key: str, ocr_text: str, page_num: int) -> Tuple[str, Dict[str, int]]:
    ...
    md = _neutralize_generated_headings(md)
...
def process_page_with_cache(pdf_path: str, page_num: int, api_key: str, is_first_page: bool = False) -> Tuple[str, Dict[str, Any]]:
    ...
    stats_core = {
        ...
        "render_dpi": RENDER_DPI,
        "pipeline": {
            "version": PHASE1_PIPELINE_VERSION,
