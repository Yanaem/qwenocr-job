...
OCR_EMPTY_RETRIES = max(0, _env_int("OCR_EMPTY_RETRIES", 2))  # retries supplémentaires
...
# Cache explicite Qwen : uniquement les longs prompts statiques sont mis en cache.
# Les images et le contenu propre à chaque page restent toujours hors cache.
...
def load_progress(
    ...
    try:
        ...
        # Compatibilité avec l'ancien format local. Il n'est repris que si aucun
        # contrôle d'identité du PDF n'est demandé.
        if isinstance(data, dict):
            if expected_source_id is not None or expected_page_count is not None:
                _log("⚠️ Ancien checkpoint ignoré : identité du PDF non vérifiable.")
                return {}
            return data
        return {}
    except Exception:
            ...
            if r.status_code == 200:
                ...
                if not text and output_tokens > 0:
                    ...
                    if reasoning_text and allow_no_think_fallback and enable_thinking is not False and _supports_thinking_toggle(model):
                        ...
                        return fallback_text, _merge_token_stats(stats, fallback_stats)
...
def ocr_page_with_vl(api_key: str, pdf_path: str, page_num: int) -> Tuple[str, Dict[str, Any]]:
    ...
    # Les retries de contenu reconstruisent volontairement la partie dynamique
    # de la requête. Le prompt OCR statique reste identique et bénéficie du cache.
    last_text = ""
    ...
    for k in range(0, OCR_EMPTY_RETRIES + 1):
        if k == 0:
            page_instruction = (
                ...
                "Retourne uniquement le texte OCR brut."
            )
        else:
            previous_length = len(last_text.strip())
            page_instruction = (
                f"Tentative OCR de secours {k}/{OCR_EMPTY_RETRIES} pour la page {page_num}. "
                f"La tentative précédente n'a produit que {previous_length} caractères, "
                "ce qui est anormalement court. Relis l'intégralité de l'image depuis le début, "
                "sans te fonder sur la réponse précédente, "
                "sans résumer, sans omettre les petits caractères, les tableaux, les montants "
                "ou les mentions de bas de page. Applique exactement toutes les règles du prompt "
                "OCR. Retourne uniquement le texte OCR brut."
            )
        ...
        last_text = text
        aggregate_stats.update(_merge_token_stats(aggregate_stats, stats))
        ...
        _log(
            f"⚠️ Page {page_num}: OCR trop court ({len(text.strip())} chars, "
            f"out={stats.get('output_tokens', 0)} tokens). Preview='{preview}'"
        )

        if k < OCR_EMPTY_RETRIES:
            time.sleep(OCR_EMPTY_RETRY_SLEEP)

    # Libération immédiate par comptage de références, sans parcours global du heap.
    del messages
    del data_url
    del image_b64
    ...
    _log(f"✅ Page {page_num}: OCR OK ({aggregate_stats.get('total_tokens', 0)} tokens cumulés)")
    return last_text, aggregate_stats
...
def markdown_from_ocr(api_key: str, ocr_text: str, page_num: int) -> Tuple[str, Dict[str, int]]:
    ...
    md, stats = _call_chat_with_cache_prime(
        cache_name="markdown_static_prompt",
        api_key=api_key,
        model=MODEL_MD,
        messages=messages,
